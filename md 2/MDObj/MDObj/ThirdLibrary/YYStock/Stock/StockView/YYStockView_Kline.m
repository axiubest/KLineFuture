...
@property (nonatomic,assign) CGFloat HKFlag;
@property (nonatomic,assign) CGFloat HKFlagTwo;
...
- (void)reDrawWithLineModels:(NSArray <id<YYLineDataModelProtocol>>*) lineModels {
    ...
    self.HKFlag = self.stockScrollView.contentSize.width;
    [self layoutIfNeeded];
    ...
    if (self.lineModels.count > 0) {
        if (self.HKFlag) {
            self.stockScrollView.contentOffset = CGPointMake(self.stockScrollView.contentSize.width - self.HKFlag, self.stockScrollView.contentOffset.y);
        }else{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    //提前加载数据也可以， 不必滑到头再进行加载
    
    
    if (contentXoffset < 0) {
        //如果滑动到此处，加载当前数据完毕，需要进行加载更多处理
        
        if (self.HKFlagTwo == scrollView.contentSize.width) {
            return;
        }
        self.HKFlagTwo = scrollView.contentSize.width;
        
        static NSInteger i=0;
        
        
        [[NSNotificationCenter defaultCenter] postNotificationName:ScrollEnd object:nil];
    }
    [self setNeedsDisplay];
}

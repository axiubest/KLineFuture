...
- (void)request {
    ...
    if ([XIU_Login isLogin]) {
        user_id_type = [XIU_Login userId];
    }else {
        user_id_type =  @"-1";
    }
    [[XIU_NetAPIClient sharedJsonClient] requestJsonDataWithPath:API_XG withParams:@{@"patternId":[NSString stringWithFormat:@"%ld", _Id],@"userId":user_id_type} withMethodType:Post andBlock:^(id data, NSError *error) {

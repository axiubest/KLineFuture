...
- (void)setHeaderDic:(NSMutableDictionary *)dic {
    if (![XIU_Login isLogin]) {
        ...
        likeBtnType = [XIU_Login userId];
    }
    
    if ([dic objectForKey:@"Id"]) {
        csId = [dic objectForKey:@"Id"];
        [[XIU_NetAPIClient sharedJsonClient] requestJsonDataWithPath:API_ChooseStockLike withParams:@{@"userId":likeBtnType,@"csId":[dic objectForKey:@"Id"]} withMethodType:Post andBlock:^(id data, NSError *error) {
...
//                后台参数为空
    ...
    _detialLab.text = [dic objectForKey:@"Description"];
    ...
    //折线图赋值
    [_secondView setData:dic];
    
    [_firstView setData:dic];

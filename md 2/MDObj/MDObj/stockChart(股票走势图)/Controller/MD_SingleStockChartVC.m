...
#import "MD_SingleStockReportCell.h"
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    if ([[resultDic allKeys] containsObject:@"showapi_res_body"]) {//分时图
        ...
        _numLab.text =[NSString stringWithFormat:@"%@%%", [NSString stringWithFormat:@"%.2f", [resultDic[@"diff_rate"] floatValue]]] ;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (SegmentType == SingleStockSegmentAtyle_ResearchReport) {
        return 1 + self.reportListArray.count;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (SegmentType == SingleStockSegmentAtyle_Information ||SegmentType == SingleStockSegmentAtyle_Notice) {
        ...
        cell.img.hidden = YES;
        cell.titleConstraint.constant = 20;
        return cell;
    }
    if (SegmentType == SingleStockSegmentAtyle_ResearchReport) {
        if (indexPath.row == 0) {
            MD_SingleStockReportCell *cell = [tableView dequeueReusableCellWithIdentifier:[MD_SingleStockReportCell XIU_ClassIdentifier]];
            [cell configUI];
            return cell;
        }else {
            MD_SingleStockInformationCell *cell = [tableView dequeueReusableCellWithIdentifier:[MD_SingleStockInformationCell XIU_ClassIdentifier]];
            cell.img.hidden = NO;
            cell.titleConstraint.constant = 70;
            cell.titile.text = self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"Title"];
            cell.time.text = self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"TimeBefore"];
            
            NSString *imgName = [self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"Rating"] isEqualToString:@"买入"] ? @"diagnose_report_icon_buy" : [self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"Rating"] isEqualToString:@"增持"] ? @"diagnose_report_icon_add" : [self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"Rating"] isEqualToString:@"持有"] ? @"diagnose_report_icon_possess" : [self.reportListArray[indexPath.row > self.reportListArray.count - 1 ? self.reportListArray.count - 1 : indexPath.row - 1][@"Rating"] isEqualToString:@"减持"] ? @"diagnose_report_icon_reduce" : @"diagnose_report_icon_sale";
...
- (void)loadTableViewCell {
    [_XIUTableView registerNib:[MD_SingleStockReportCell XIU_ClassNib] forCellReuseIdentifier:[MD_SingleStockReportCell XIU_ClassIdentifier]];
...
#pragma mark 研报
- (void)requestReportInformation {
    
//    API_GetReportList
    [[XIU_NetAPIClient sharedJsonClient]requestJsonDataWithPath:API_GetReport withParams:@{@"type" : reportSumType, @"Code":_CodeString} withMethodType:Post andBlock:^(id data, NSError *error) {
...
- (void)requestReportList {
    [[XIU_NetAPIClient sharedJsonClient]requestJsonDataWithPath:API_GetReportList withParams:@{@"code":_CodeString, @"startIndex":@"1", @"endIndex":@"10"} withMethodType:Post andBlock:^(id data, NSError *error) {
        for (NSDictionary *dic in data[@"Reportses"]) {
            
            NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithCapacity:10];
            [dict setObject:dic[@"TimeBefore"] forKey:@"TimeBefore"];
            
            [dict setObject:dic[@"Rating"] forKey:@"Rating"];
            [dict setObject:dic[@"Title"] forKey:@"Title"];
            [dict setObject:dic[@"Id"] forKey:@"Id"];
            [self.reportListArray addObject:dic];
        }
        [self reload];
    }];
}
...
- (void)requestMore:(NSNotification *)sender {
    ...
    if (self.stock.currentIndex == 1) {//日
        ...
        for (YYLineDataModel *mo in _tmpDayArray) {
            NSLog(@"%@", mo.Day);
        }
        
        [[MD_RequestManager sharedManager]request_MoreKlineOfDayWithDate:DateTime CodeString:_CodeString Block:^(id data, NSError *error) {
     ...
     if (self.stock.currentIndex == 2) {//week
         NSString *DateTime = [[_tmpWeekArray[0] Day] stringByReplacingOccurrencesOfString:@"-" withString:@""];
         for (YYLineDataModel *mo in _tmpWeekArray) {
             NSLog(@"%@", mo.Day);
         }
         
         [[MD_RequestManager sharedManager]request_MoreKlineOfWeekWithDate:DateTime CodeString:_CodeString Block:^(id data, NSError *error) {
             
             for (YYLineDataModel *obj in data) {
                 [_tmpWeekArray insertObject:obj atIndex:0];
             }
             
             [self.stockDatadict setObject:_tmpWeekArray forKey:@"weekList"];
    ...
    if (self.stock.currentIndex == 3) {
        NSString *DateTime = [[_tmpMonthArray[0] Day] stringByReplacingOccurrencesOfString:@"-" withString:@""];
        for (YYLineDataModel *mo in _tmpMonthArray) {
            NSLog(@"%@", mo.Day);
        }
        
        [[MD_RequestManager sharedManager]request_MoreKlineOfMonthWithDate:DateTime CodeString:_CodeString Block:^(id data, NSError *error) {
            
            for (YYLineDataModel *obj in data) {
                [_tmpMonthArray insertObject:obj atIndex:0];
            }
            
            [self.stockDatadict setObject:_tmpMonthArray forKey:@"monthList"];
...
-(NSMutableArray *)reportListArray {
    if (!_reportListArray) {
        _reportListArray = [NSMutableArray array];
    }

...
- (void)setHeaderView {
    ...
    UIView *headerV = [[UIView alloc] initWithFrame:CGRectMake(0, 0, KWIDTH, 64)];
    headerV.backgroundColor = [UIColor colorWithHexString:@"378AD6"];
    UIImageView *v = [[UIImageView alloc] init];
    v.layer.cornerRadius = 5.f;
    v.layer.masksToBounds = YES;
    v.image = [UIImage imageNamed:@"home_searchbar_top"];
    [headerV addSubview:v];
    v.userInteractionEnabled = YES;
    v.backgroundColor = [UIColor whiteColor];
    [v mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(headerV).with.offset(15);
        make.right.equalTo(headerV).with.offset(-15);
        make.height.mas_equalTo(@30);
        make.bottom.equalTo(headerV).with.offset(-7);

    }];
    [headerV bk_whenTapped:^{
...
- (void)clickKlinePKBtn {
    ...
    NSString *tmp = [NSString stringWithFormat:@"http://wx.shangjin666.cn/index.html#/klinearena?userId=2%@",[XIU_Login userId]];
...
-(UITableView *)XIUTableView {
    if (!_XIUTableView) {
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,-30, KWIDTH, self.view.height - 10) style:UITableViewStyleGrouped];

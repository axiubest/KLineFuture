...
@interface MD_QuotationHSMoreVC ()<UITableViewDelegate, UITableViewDataSource,UIScrollViewDelegate>
{
...
@implementation MD_QuotationHSMoreVC
...
- (void)viewDidLoad {
    ...
    downPresent = NO;
    switch (_type) {
        case MD_StockListStyle_Up:
            self.navigationItem.title = @"涨幅榜";
            xsortField = @"diff_money";
            xsortOrder = @"DESC";
            break;
        case MD_StockListStyle_Down:
            self.navigationItem.title = @"跌幅榜";
            xsortField = @"diff_money";
            xsortOrder = @"ASC";
            
            break;
        case MD_StockListStyle_Change:
            self.navigationItem.title = @"换手率榜";
            xsortField = @"turnover";
            xsortOrder = @"DESC";
            
            break;
        case MD_StockListStyle_OK:
            self.navigationItem.title = @"成交额榜";
            xsortField = @"tradeAmount";
            xsortOrder = @"DESC";
...
- (void)requestWithType:(NSInteger)type{
    [[XIU_NetAPIClient sharedJsonClient]requestJsonDataWithPath:API_GetRanking withParams:@{@"APPID":APPID,@"timestamp":XIU_Timestamp,@"signed":XIU_Signed,@"type":[NSNumber numberWithInteger:type],@"startIndex":[NSNumber numberWithInteger:xstartIndex], @"endIndex":[NSNumber numberWithInteger:xendIndex],@"field":xsortField,@"order":xsortOrder} withMethodType:Post andBlock:^(id data, NSError *error) {

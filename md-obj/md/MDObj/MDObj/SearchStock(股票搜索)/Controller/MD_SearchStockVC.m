...
@interface MD_SearchStockVC ()<UISearchBarDelegate,UITableViewDelegate, UITableViewDataSource>
...
@implementation MD_SearchStockVC
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (tableView == self.historyTableView) {
        ...
        [b setTitle:@"清空历史纪录" forState:UIControlStateNormal];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.XIUTableView) {
        ...
        if (self.historyListData.count == 0) {
            ...
            [XIU_NSUserDefaut addObject:self.historyListData WithObjName:history_search];
        ...
        vc.CodeString = [self.dataSource[indexPath.row] objectForKey:@"Code"];
         vc.CodeName = [self.dataSource[indexPath.row] objectForKey:@"Name"];
        [self.navigationController pushViewController:vc animated:YES];
    }if (tableView == self.historyTableView) {
        MD_SingleStockChartVC *vc = [[MD_SingleStockChartVC alloc] init];
        vc.CodeString = [self.historyListData[indexPath.row] objectForKey:@"Code"];
        vc.CodeName = [self.historyListData[indexPath.row] objectForKey:@"Name"];
        [self.navigationController pushViewController:vc animated:YES];
    }
}
...
-(NSMutableArray *)historyListData
{
    if (!_historyListData) {
        ...
        NSMutableArray *arr = (NSMutableArray*)[XIU_NSUserDefaut getObjectWithObjName:history_search];

...
- (void)updateDrawModels {
    ...
    maxValue = [[[self.drawLineModels valueForKeyPath:@"nowPrice"] valueForKeyPath:@"@max.floatValue"] floatValue];
    minValue = [[[self.drawLineModels valueForKeyPath:@"nowPrice"] valueForKeyPath:@"@min.floatValue"] floatValue];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return section == 1 ? 5:16;
}

- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        UILabel *lab = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 35, 18)];
        lab.center = self.center;
        lab.font = [UIFont systemFontOfSize:11];
        lab.text = @"五档";
        lab.textAlignment = 1;
        lab.layer.borderColor = [UIColor YYStock_TimeVolumeColor].CGColor;
        lab.layer.borderWidth = .5f;
        lab.layer.masksToBounds = YES;
        lab.layer.cornerRadius = 3.f;
        lab.textColor = [UIColor YYStock_TimeVolumeColor];
        return lab;
    }
    if (section == 1) {

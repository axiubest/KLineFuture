...
- (void)drawTimeLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    [drawText drawInRect:drawRect withAttributes:attribute];
    NSLog(@"%@", self.selectedModel);
    
    CGFloat volumes = [self.selectedModel volume];
    attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor YYStock_topBarSelectedTextColor]};
    //尝试转为万手
    CGFloat wVolume = volumes/10000.f;
    if (wVolume > 1) {
        //尝试转为亿手
        ...
        drawText = [NSString stringWithFormat:@"%.2f  手",volumes];
    }
    textRect = [self rectOfNSString:drawText attribute:attribute];
...
- (NSArray *)drawTimeLineTexts {
    return @[
             [NSString stringWithFormat:@"%.2f",[((id<YYStockTimeLineProtocol>)self.selectedModel).nowPrice floatValue]],
             [NSString stringWithFormat:@"%.2f%%",([[((id<YYStockTimeLineProtocol>)self.selectedModel) nowPrice] floatValue] - [((id<YYStockTimeLineProtocol>)self.selectedModel) AvgPrice])*100/[((id<YYStockTimeLineProtocol>)self.selectedModel) AvgPrice]],

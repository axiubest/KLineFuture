...
@property (nonatomic, weak) NSArray *drawLineModels;
@end
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i <self.drawPositionModels.count ; i++) {
        YYVolumePositionModel *pModel = self.drawPositionModels[i];
        
        //绘制成交量线
        //成交量颜色判断
        if (_drawLineModels[i - 1 < 0 ? i : i-1] > _drawLineModels[i]) {
              CGContextSetStrokeColorWithColor(ctx, [UIColor colorWithHexString:@"1CB54A"].CGColor);
        }else {
            CGContextSetStrokeColorWithColor(ctx, [UIColor YYStock_TimeVolumeColor].CGColor);
